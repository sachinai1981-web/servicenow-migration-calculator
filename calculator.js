...
const PERSONAL_EMAIL_DOMAINS = [
  ...
  'me.com', 'mac.com', 'aol.com', 'protonmail.com',
...
const MIGRATION_BASE_RATE         = 0.30;
...
function calculate(inputs) {
  ...
  const totalMigrationCost = (baseMigration + staffRetraining) * timelineMultiplier;
  ...
  const freshserviceAnnualCost = Math.max(rawFreshserviceCost, FRESHSERVICE_MINIMUM_ANNUAL);

  // Summary figures
  const year1Total     = totalMigrationCost + freshserviceAnnualCost;
  const threeYearSavings = (servicenowAnnualCost * 3) - (totalMigrationCost + freshserviceAnnualCost * 3);
  const totalThreeYearSpend = totalMigrationCost + freshserviceAnnualCost * 3;
  const roi = totalThreeYearSpend > 0
...
function getInputValues() {
  return {
    ...
    staffCount:    parseInt(document.getElementById('staff-count').value, 10)  || 0,
...
function renderResults(results) {
  const { servicenowAnnualCost, totalMigrationCost, freshserviceAnnualCost, year1Total, threeYearSavings, roi } = results;
  ...
  setResultText('result-freshservice-cost',  formatCurrency(freshserviceAnnualCost));
  setResultText('result-year1-total',        formatCurrency(year1Total));

  if (threeYearSavings < 0) {
    setResultText('result-3yr-savings', 'Migration not recommended at this scale');
